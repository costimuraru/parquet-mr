...
import org.apache.parquet.proto.test.MapProtobuf.Log;
...
public class WriteTest5 {
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < 1; i++) {
      ...
      Log message = Log.newBuilder()
        .setIp("ip")
        .putAdditional("key1", "value1")
        .putAdditional("key2", "value2")
        .build();
    ...
    try {
      Files.delete(Paths.get("/tmp/test.map.parquet"));
    } catch(Exception e) {}

    Path outputPath = new Path("/tmp/test.map.parquet");
    writeMessages(Log.class, outputPath, messages.toArray(new MessageOrBuilder[messages.size()]));
